...
export const TrainingCard = ({ title, slug }: TrainingCardPropsType) => {
  const href = slug ? `/trainings/${slug}` : '#';
  ...
  return (
    <Link
      ...
      <span className="text-[14px] font-medium text-gray-900 sm:text-[16px]">{title}</span>
        <svg
          className="h-5 w-5 shrink-0 text-gray-400 transition-transform group-hover:translate-x-1"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2}
