...
const SUB_ITEMS = [
  ...
  {
    name: '«Безпечна громада»',
    icon: ShieldExclamationIcon,
    description: 'Безпека та захист громади',
    color: 'text-emerald-600',
    bgColor: 'bg-emerald-50',
    hoverBg: 'group-hover/sub:bg-emerald-100',
    dotColor: 'bg-emerald-500',
  },
  {
    name: '«Заможна громада»',
    icon: BanknotesIcon,
    description: 'Економічний розвиток',
    color: 'text-blue-600',
    bgColor: 'bg-blue-50',
    hoverBg: 'group-hover/sub:bg-blue-100',
...
/* ── Option B: Compact Pill-style Sub-items ── */
function OptionBDropdown() {
  const [open, setOpen] = useState(true);
  ...
  return (
    <div className="w-full max-w-md rounded-3xl bg-white shadow-lg ring-1 ring-gray-900/5">
      <div className="p-4">
        ...
        {/* OPTION B: Compact pills in a wrapping grid */}
        {open && (
          <div className="ml-[4.25rem] flex flex-wrap gap-2 pb-3 pt-1">
            {SUB_ITEMS.map((item) => (
              <a
                key={item.name}
                href="#"
                className={`group/sub inline-flex items-center gap-x-2 rounded-full ${item.bgColor} px-3.5 py-1.5 transition-all duration-150 hover:shadow-sm hover:ring-1 hover:ring-gray-200`}
              >
                <item.icon className={`size-3.5 ${item.color}`} />
                <span className={`text-xs font-semibold ${item.color}`}>{item.name}</span>
              </a>
            ))}
          </div>
        )}
...
/* ── Option C: Minimal with Dot Indicators ── */
  ...
  return (
    <div className="w-full max-w-md rounded-3xl bg-white shadow-lg ring-1 ring-gray-900/5">
      <div className="p-4">
        ...
        {/* OPTION C: Clean minimal list with colored dot indicators and descriptions */}
        {open && (
          <div className="ml-9 space-y-0.5 border-l-2 border-gray-100 py-1">
            {SUB_ITEMS.map((item) => (
              <a
                key={item.name}
                href="#"
                className="group/sub flex items-start gap-x-3 rounded-r-lg py-2.5 pl-5 pr-3 transition-colors duration-150 hover:bg-gray-50"
              >
                <span className={`mt-1.5 size-2 flex-none rounded-full ${item.dotColor}`} />
                <div>
                  <span className="text-sm font-medium text-gray-700 group-hover/sub:text-gray-900">{item.name}</span>
                  <p className="text-xs text-gray-400 group-hover/sub:text-gray-500">{item.description}</p>
                </div>
              </a>
            ))}
          </div>
      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="mb-16">
          ...
          <p className="mt-3 max-w-2xl text-base text-gray-600">
            {
              'Поточні дочірні елементи під «Мегапроект Вдома» виглядають як простий текст без візуального оформлення. Нижче представлено поточний стан та три варіанти редизайну.'
            }
          </p>
        </div>
        ...
        <div className="grid gap-12 lg:grid-cols-2">
          ...
          <div>
            <div className="mb-4 flex items-center gap-x-3">
              ...
              <span className="text-xs font-medium text-gray-400">Рекомендований</span>
            </div>
            <p className="mb-6 text-sm text-gray-500">
          ...
          <div>
            ...
            <p className="mb-6 text-sm text-gray-500">
              Компактні пілюлі (pills) з кольоровими акцентами. Економить вертикальний простір, виглядає тегоподібно.
            </p>
            <OptionBDropdown />
            <div className="mt-4 rounded-xl border border-gray-200 bg-white p-4">
              <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-gray-400">
              ...
              <ul className="space-y-1.5 text-xs text-gray-600">
                <li>
                  {'flex flex-wrap gap-2 на контейнері'}
                </li>
                <li>
                  {'rounded-full з кольоровим bg для кожного pill (bg-amber-50, bg-emerald-50, ...)'}
                </li>
                <li>
                  {'Іконка size-3.5 + текст text-xs font-semibold'}
                </li>
                <li>
                  {'hover:shadow-sm hover:ring-1 hover:ring-gray-200 для зворотнього зв\'язку'}
                </li>
              </ul>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="mb-6 text-sm text-gray-500">
              Мінімалістичний список з кольоровими крапками та коротким описом. Найчистіший варіант.
            </p>
            <OptionCDropdown />
            <div className="mt-4 rounded-xl border border-gray-200 bg-white p-4">
              <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-gray-400">
              ...
              <ul className="space-y-1.5 text-xs text-gray-600">
                <li>
                  {'border-l-2 border-gray-100 + pl-5 для візуальної ієрархії'}
                </li>
                <li>
                  {'Кольорова крапка size-2 rounded-full (bg-amber-500, bg-emerald-500, ...)'}
                </li>
                <li>
                  {'Опис text-xs text-gray-400 під назвою для додаткового контексту'}
                </li>
                <li>
                  {'Потребує додати поле description до кожного суб-елемента в константах'}
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-8 md:grid-cols-2">
            ...
            <div>
              ...
              <ul className="mt-2 space-y-1.5 font-mono text-xs text-gray-600">
                ...
                <li>{'@heroicons/react/24/outline: ShieldExclamationIcon'}</li>
                <li>{'@heroicons/react/24/outline: BanknotesIcon'}</li>
                <li>{'@heroicons/react/24/outline: SparklesIcon'}</li>
                <li>{'@heroicons/react/20/solid: ChevronRightIcon (Option A)'}</li>
              </ul>
            </div>
            <div>
